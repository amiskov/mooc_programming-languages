...
(cond [e1a e1b]
      [e2a e2b]
      ...
      [eNa eNb]) ; Good style — должно быть `#t`
```
...
Если так получится, что ни одно условие не выполнится и никакой результат не вернется, то `cond` вернет что-то странное и это плохо. Нужно следить, чтобы условия выполнялись. Поэтому хорошим тоном считается, чтобы последнее условие обязательно было `#t`:
